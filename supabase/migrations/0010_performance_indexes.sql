...
CREATE INDEX idx_reviews_cycle_contractor ON reviews(cycle_id, contractor_id);

-- reviews: filtradas por author_id + status (badges do layout para client_rep)
CREATE INDEX idx_reviews_author_status ON reviews(author_id, status);
...
CREATE INDEX idx_allocations_contractor ON allocations(contractor_id);
CREATE INDEX idx_allocations_client_active ON allocations(client_id, ended_on);

-- contractor_history: filtrada por cycle_id + contractor_id (dashboard admin)
CREATE INDEX idx_contractor_history_cycle ON contractor_history(cycle_id, contractor_id);
